  ...
  return (
    <DropdownMenuPrimitive.Portal>
      <DropdownMenuPrimitive.Content
        ...
        className={cn(
          "m3-content z-50 rounded-xl bg-popover/95 backdrop-blur-xl text-popover-foreground shadow-pop border border-border outline-none overflow-hidden relative py-0",
...
>(({ className, inset, children, delayDuration = 250, enterAnimation = true, asChild = false, ...props }, ref) => {
  ...
  const baseClassName = cn(
    "group relative flex cursor-pointer select-none items-stretch px-0 min-h-[44px] text-sm font-medium tracking-[0.01em] outline-none transition-colors",
...
>(({ className, inset, ...props }, ref) => (
  <DropdownMenuPrimitive.Label
    ref={ref}
    className={cn(
      "px-5 py-3 text-[10px] font-black tracking-[0.15em] text-primary/80 uppercase m3-item-enter",
      inset && "pl-12",
      className,
    )}
    {...props}
...
const DropdownMenuInternalBack = () => {
  ...
  return (
    <DropdownMenuItem
      ...
      style={{ "--m3-stagger": 0 } as React.CSSProperties}
    >
...
>(({ id, children, className, ...props }, ref) => {
  ...
  const staggeredChildren = React.Children.map(children, (child, index) => {
    if (React.isValidElement(child))
      return React.cloneElement(child, {
        style: { ...(child.props as any).style, "--m3-stagger": id === "main" ? index : index + 1 } as React.CSSProperties,
      } as any);
    return child;
  });
