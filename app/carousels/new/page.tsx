...
export default function NewCarouselWizard() {
  ...
  return (
    <div className="min-h-screen bg-bg">
      {/* Header */}
      <div className="h-12 bg-bg-surface border-b border-border flex items-center px-4 gap-3">
          ...
          <div key={s.key} className="flex items-center gap-1">
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-[11px] font-semibold transition-colors ${
                step === s.key ? 'bg-accent text-white' : 'bg-bg-surface text-muted border border-border'
              }`}
            >
              {i + 1}. {s.label}
            </button>
      ...
      <div className="max-w-2xl mx-auto px-4 pb-8">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <div className="text-[11px] text-muted">Выбери видео из ContentOS с готовым транскриптом</div>
              </button>
              <button
              ...
              >
                ...
                <div className="text-[11px] text-muted">Вставь текст статьи, заметки или транскрипт</div>
              </button>
            </div>
            ...
            {sourceType === 'video' && (
              <div className="space-y-2">
                <label className="text-[11px] font-semibold text-muted">Выбери видео</label>
                <div className="space-y-1.5 max-h-[300px] overflow-y-auto">
                  {videos.length === 0 ? (
                    <div className="text-xs text-muted py-4 text-center">Нет видео с транскриптами</div>
                  ) : (
                    videos.map(v => (
                      <button
                        key={v.id}
                      ...
                      >
                        <Play className="w-4 h-4 mt-0.5 shrink-0 text-muted" />
            ...
            {sourceType === 'text' && (
              <div className="space-y-3">
                <div>
                  <label className="text-[11px] font-semibold text-muted mb-1 block">Тема карусели</label>
                  <input
                    value={topic}
                    onChange={e => setTopic(e.target.value)}
                    placeholder="Например: 5 ошибок в переговорах о зарплате"
                    className="w-full bg-bg-surface rounded-lg border border-border px-3 py-2 text-sm outline-none focus:border-accent"
                  />
                </div>
                <div>
                  <label className="text-[11px] font-semibold text-muted mb-1 block">Исходный текст</label>
                  <textarea
                    value={sourceText}
                    onChange={e => setSourceText(e.target.value)}
                    rows={8}
                    placeholder="Вставь текст статьи, заметки, конспект подкаста..."
                    className="w-full bg-bg-surface rounded-lg border border-border px-3 py-2 text-sm outline-none focus:border-accent resize-none"
                  />
                  <div className="text-[10px] text-muted mt-1">{sourceText.length} символов</div>
                </div>
              </div>
            )}
        ...
        {step === 'style' && (
          <div className="space-y-5">
            ...
            {/* Brand preset */}
            <div>
              <div className="text-[10px] font-semibold tracking-widest uppercase text-muted mb-2">Визуальный бренд</div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[11px] font-semibold text-muted mb-1 block">Тон</label>
                  ...
                  {tones.map(t => (
                    <button
                      ...
                      className={`px-2 py-1 rounded-full text-[10px] font-semibold border transition-colors ${
                        tone === t ? 'border-accent bg-accent text-white' : 'border-border text-muted'
                      }`}
                    >
                      {t}
                    </button>
              ...
              <div>
                <label className="text-[11px] font-semibold text-muted mb-1 block">Аудитория</label>
                <input
                  value={audience}
                  onChange={e => setAudience(e.target.value)}
                  placeholder="Предприниматели 25-40"
                  className="w-full bg-bg-surface rounded-lg border border-border px-3 py-2 text-xs outline-none focus:border-accent"
                />
              </div>
            </div>
            ...
            {/* Slide count */}
            <div>
              <label className="text-[11px] font-semibold text-muted mb-1 block">Количество слайдов</label>
            ...
            <div className="border-t border-border pt-4">
              <div className="text-[10px] font-semibold tracking-widest uppercase text-muted mb-2">Голос бренда (текстовый стиль)</div>
              ...
              {voices.length > 0 && (
                <div className="space-y-1.5 mb-3">
                  ...
                  >
                    ...
                    <span className="text-muted ml-2">Экспертный, нейтральный тон</span>
                  </button>
                  {voices.map(v => (
                    <button
                      key={v.id}
                    ...
                    >
                      ...
                      {v.summary && <span className="text-muted ml-2">{v.summary}</span>}
                    </button>
                  ))}
                </div>
              )}
              ...
              <details className="group">
                ...
                <div className="mt-3 space-y-2">
                  <div className="text-[11px] text-muted">Добавь 2-5 примеров своих лучших текстов. ИИ проанализирует стиль и будет писать в том же тоне.</div>
                  <input
                    value={voiceName}
                  ...
                  {examples.map((ex, i) => (
                    <div key={i} className="flex gap-2">
                      <textarea
                        ...
                        className="flex-1 bg-bg-surface rounded-lg border border-border px-3 py-2 text-xs outline-none focus:border-accent resize-none"
                      />
                      {examples.length > 1 && (
                        <button onClick={() => setExamples(examples.filter((_, j) => j !== i))} className="text-muted hover:text-red-500">
                  ...
                  <div className="flex gap-2">
                    {examples.length < 5 && (
                      <button onClick={() => setExamples([...examples, ''])} className="flex items-center gap-1 text-xs text-muted hover:text-accent">
            ...
            <div className="flex gap-2">
              <button onClick={() => setStep('source')} className="flex-1 py-3 rounded-lg border border-border text-xs font-semibold text-muted hover:border-gray-400">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-border text-xs font-semibold text-muted hover:border-accent hover:text-accent disabled:opacity-30"
              >
            ...
            {isGenerating && (
              <div className="flex flex-col items-center py-12 gap-3">
                ...
                <div className="text-xs text-muted">Claude создаёт слайды, подпись и хэштеги</div>
              </div>
            )}
              ...
              <>
                  ...
                  {slides.map((slide, i) => {
                    ...
                    return (
                      <div key={i} className="bg-bg-surface border border-border rounded-xl p-4">
                        ...
                        {slide.subtitle && <div className="text-xs text-muted mb-1">{slide.subtitle}</div>}
                        {slide.lead && <div className="text-xs text-muted mb-1">{slide.lead}</div>}
                        {slide.bold && <div className="text-xs font-bold mb-1">{slide.bold}</div>}
                        {slide.body && <div className="text-xs text-muted">{slide.body}</div>}
                        {slide.example && (
                          <div className="mt-2 pt-2 border-t border-border">
                            <span className="text-[10px] font-semibold text-muted">ПРИМЕР: </span>
                            <span className="text-xs text-muted italic">{slide.example}</span>
                          </div>
                        )}
                      </div>
                    )
                ...
                {caption && (
                  <div className="bg-bg-surface border border-border rounded-xl p-4">
                    <div className="text-[10px] font-semibold tracking-widest uppercase text-muted mb-2">Подпись к посту</div>
                    <div className="text-xs text-muted whitespace-pre-wrap">{caption}</div>
                  </div>
                )}

                <div className="flex gap-2">
                  <button onClick={() => setStep('style')} className="flex-1 py-3 rounded-lg border border-border text-xs font-semibold text-muted hover:border-gray-400">
