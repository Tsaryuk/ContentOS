...
// per project. Usage rows not linked to a video fall under "Без проекта".
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const { data, error } = await supabaseAdmin
    .from('ai_usage')
    .select(`
      provider, model, task, input_tokens, output_tokens, units, cost_usd,
      created_at, video_id,
      yt_videos!left (
        channel_id,
        yt_channels!left ( project_id, title, projects!left ( name, color ) )
      )
    `)
  ...
  const byProject  = new Map<string, { name: string; color: string | null; count: number; cost: number }>()
  ...
  for (const r of rows) {
    ...
    const proj = r.yt_videos?.yt_channels?.projects
    const projId = r.yt_videos?.yt_channels?.project_id ?? 'unattributed'
    const projName = proj?.name ?? 'Без проекта'
    const projColor = proj?.color ?? null
    const pr = byProject.get(projId) ?? { name: projName, color: projColor, count: 0, cost: 0 }
    pr.count += 1; pr.cost += cost
    byProject.set(projId, pr)
  }

  const sortedBy = <V extends { cost: number }>(m: Map<string, V>) =>
  ...
  return NextResponse.json({
    ...
    byProvider: sortedBy(byProvider),
    byTask:     sortedBy(byTask),
    byModel:    sortedBy(byModel),
    byProject:  sortedBy(byProject),
