...
// Bulk-update descriptions of a channel's already-published videos.
//
// Body:
//   { channelId, op: 'rebuild_block' }          — re-apply the channel link block
//   { channelId, op: 'replace_url', fromUrl, toUrl } — swap one link for another
//
// Each video gets an `update_description` job (description-only YouTube write,
// no re-approve), staggered 3s to stay within API quota.
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return handleApiError(err, { route: '/api/youtube/bulk-update-links', userId: auth.userId })
  }
}
