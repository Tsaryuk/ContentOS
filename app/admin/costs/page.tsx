...
export default function CostsPage() {
  ...
  if (!report) {
    return (
      <div className="p-8 max-w-5xl mx-auto flex items-center gap-2 text-muted">
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-8">
      <header className="flex items-baseline justify-between">
        <div>
          ...
          <p className="text-sm text-muted mt-1">
            {report.totalEvents} событий · покрытие ценой {report.totalEvents ? Math.round(report.eventsWithCost / report.totalEvents * 100) : 0}%
          </p>
        </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <Breakdown title="По провайдеру" rows={report.byProvider} />
        <Breakdown title="По задаче" rows={report.byTask} />
            ...
            <tbody>
              {report.recent.map((r, i) => (
                <tr key={i} className="border-b border-border/30 last:border-0 hover:bg-bg/40">
                  ...
                  <td className="px-3 py-2 text-muted">{r.provider}</td>
                  <td className="px-3 py-2 text-cream">{r.model}</td>
                  <td className="px-3 py-2 text-muted">{r.task ?? '—'}</td>
