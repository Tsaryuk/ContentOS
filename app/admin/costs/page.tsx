...
export default function CostsPage() {
  ...
  if (!report) {
    return (
      <div className="p-8 max-w-6xl mx-auto flex items-center gap-2 text-muted-foreground">
    ...
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link href="/admin" className="p-2 rounded-lg bg-card hover:bg-background/70 text-muted-foreground hover:text-foreground">
            <ArrowLeft className="w-4 h-4" />
          </Link>
          <div>
            <h1 className="text-2xl font-semibold text-foreground">Стоимость AI</h1>
          ...
          {RANGES.map(r => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                days === r.days ? 'bg-accent text-white' : 'bg-card text-muted-foreground hover:text-foreground'
              }`}
            >
              {r.label}
            </button>
      ...
      <section className="rounded-xl border border-border bg-card p-6">
        <div className="text-xs text-muted-foreground/60 uppercase tracking-wider mb-1">Всего за период</div>
      ...
      <section>
        <h2 className="text-sm font-medium text-muted-foreground mb-3">По дням</h2>
      ...
      {report.byProject.length > 0 && (
        <section>
          <h2 className="text-sm font-medium text-muted-foreground mb-3">По проектам</h2>
              ...
              return (
                <div key={pr.key}>
                  <div className="flex justify-between text-xs mb-1">
                    ...
                    <span className="text-muted-foreground tabular-nums">${pr.cost.toFixed(2)} · {pr.count} событий</span>
                  </div>
      ...
      <section>
        <h2 className="text-sm font-medium text-muted-foreground mb-3">Последние 50 событий</h2>
        <div className="rounded-xl border border-border bg-card overflow-hidden">
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border text-muted-foreground/60 uppercase tracking-wider">
            ...
            <tbody>
              {report.recent.map((r, i) => (
                <tr key={i} className="border-b border-border/30 last:border-0 hover:bg-background/40">
                  ...
                  <td className="px-3 py-2 text-foreground">{labelTask(r.task)}</td>
                  <td className="px-3 py-2 text-muted-foreground">{labelProvider(r.provider)}</td>
                  <td className="px-3 py-2 text-muted-foreground text-[10px]">{r.model}</td>
                  <td className="px-3 py-2 text-right text-muted-foreground tabular-nums">{r.input_tokens ?? '—'}</td>
                  <td className="px-3 py-2 text-right text-muted-foreground tabular-nums">{r.output_tokens ?? '—'}</td>
                  <td className="px-3 py-2 text-right text-muted-foreground tabular-nums">{r.units ?? '—'}</td>
...
function Breakdown({ title, rows, showTokens }: BreakdownProps) {
  ...
  return (
    <section className="rounded-xl border border-border bg-card p-4">
      <h3 className="text-xs font-medium text-muted-foreground/60 uppercase tracking-wider mb-3">{title}</h3>
      <div className="space-y-2">
        {rows.slice(0, 10).map(r => {
          ...
          return (
            <div key={r.key}>
              <div className="flex justify-between text-xs mb-1">
                <span className="text-foreground truncate pr-2">{r.key}</span>
                <span className="text-muted-foreground tabular-nums">${r.cost.toFixed(2)}</span>
              </div>
