...
export const DEFAULT_STYLE_PRESETS: ThumbnailStylePreset[] = [
  ...
  },
  {
    id: 'silhouette',
    name: 'Силуэты',
    description: 'Контражур, чёрный фон, лица в подсветке с rim-light, белый плоский текст',
    prompt: [
      'STYLE OVERRIDE — high-contrast podcast portrait with strong rim-light contour. THIS OVERRIDE IS ABSOLUTE.',
      ...
      'Background: PURE flat BLACK (#000000). Absolutely no tint, no gradient, no atmospheric haze, no color.',
      'Lighting (KEY EFFECT): strong cool-white rim light from behind each subject — wraps the contour of the head, hair, ears, and shoulders with a bright glowing edge that visibly separates the figure from the black background. There must be a clearly visible halo / glow on the silhouette outline (especially on hair and shoulders).',
      'Faces (CRITICAL — DO NOT make them invisible): faces are CLEARLY VISIBLE and recognizable from the provided reference photos. Render eyes, nose, mouth, and skin texture with full photorealistic detail — preserve the exact face from each reference photo. The front of the face sits in soft low-key shadow but every feature reads. Eyes look directly at the camera and catch a small specular highlight. Skin tones are slightly desaturated (close to monochrome / cool grey) but not pure black-and-white — keep them as natural muted greyscale-tinted color, not stylized B&W illustration.',
      'Subject framing: head-and-shoulders portrait per person, shoulders and upper chest visible (suit / shirt / dark clothing also rim-lit on the edges). Hairline catches the strongest light.',
      'Text (CRITICAL — must be FLAT, no effects): single block of bold all-caps sans-serif (Inter Display Black / Druk Wide / Helvetica Now Black), PURE WHITE (#FFFFFF), 2 short stacked lines. The text is FLAT WHITE — NO glow, NO halo, NO outer shadow, NO gradient, NO inner highlight, NO 3D effect. Crisp solid white letters on black, that is all. Also: NO green accent, NO secondary color — both lines are pure white. Text sits in the negative space between the subjects (duo) or fills the empty side (solo).',
      'Composition: works best with the duo template — two head-and-shoulder portraits facing slightly inward, text centered in the gap between them.',
      'Mood: dramatic, prestige-podcast, cinematic. Strong contrast between deep black background, rim-lit human silhouettes, and pure white flat text.',
    ].join(' '),
  },
]
