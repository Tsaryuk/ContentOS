...
export default function CarouselsPage() {
  ...
  if (loading) return <div className="text-sm text-muted text-center py-12">Загрузка...</div>
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      {voices.length > 0 && (
        <div className="mb-6">
          <div className="text-[10px] font-semibold tracking-widest uppercase text-muted mb-2">Обученные стили</div>
          <div className="flex gap-2 flex-wrap">
            {voices.map(v => (
              <div key={v.id} className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-bg-surface border border-border text-xs">
                <Sparkles className="w-3 h-3 text-accent" />
                <span className="font-semibold">{v.name}</span>
                {v.summary && <span className="text-muted hidden sm:inline">— {v.summary}</span>}
              </div>
            ))}
          </div>
        </div>
      ...
      {carousels.length === 0 ? (
        <div className="text-center py-16">
          ...
          <div className="text-xs text-muted mb-4">Создай первую — выбери источник, настрой стиль, получи готовые слайды</div>
          <button
            onClick={() => router.push('/carousels/new')}
